...
----------------------------- */
export function createCompleteField ({Cmp = FormControl, fieldProps, required, inputProps}) {
  return function ({input, meta: {error, touched, asyncValidating}}) {
...
/* -----------------------------
  ...
  Provide button outside of FormField, but make it positioned & styled
    as InputGroup.Button
----------------------------- */
export function PseudoInputGroup ({button, children, btnInsideField=true, ...props}) {
  return <div {...props} className={cls('Pseudo-InputGroup', {
    'is-btnInsideField': btnInsideField
  })}>
    {children}
    <Col className='Pseudo-InputGroup__btnWrap' {...PseudoInputGroup.btnColConfig}> {button} </Col>
  </div>
}
PseudoInputGroup.btnColConfig = mapValues(COL_CONFIG.mainReversed, (v,k) => v - COL_CONFIG.label[k])
...
fieldComponentsInjector.schemeToFields = ({reduxFieldProps, ...props}, name) =>
  <Field 
    {...reduxFieldProps}
    key={name} 
    name={name} 
    component={createCompleteField(props)} />
